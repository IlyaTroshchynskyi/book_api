...
class BooksApiTestCase(APITestCase):
    ...
    def test_get(self):
        url = reverse('book-list')  # нужен для получения списка list маркрировка
    ...
    def test_get_search(self):
        ...
        books = Book.objects.filter(id__in=[self.book_1.id, self.book_3.id]). \
            annotate(annotated_likes=Count(Case(When(
            userbookrelation__like=True, then=1))),
            rating=Avg('userbookrelation__rate')
        ).order_by('id')
        response = self.client.get(url, data={'search': 'Author 1'})
        serializer_data = BooksSerializer(books,
                                          many=True).data
        self.assertEqual(status.HTTP_200_OK, response.status_code)
        self.assertEqual(serializer_data, response.data)
    ...
    def test_get_filter(self):
        ...
        books = Book.objects.filter(id__in=[self.book_2.id, self.book_3.id]). \
            annotate(annotated_likes=Count(Case(When(
            userbookrelation__like=True, then=1))),
            rating=Avg('userbookrelation__rate')
        ).order_by('id')
    ...
    def test_update_not_owner(self):
        ...
        self.assertEqual({'detail':
                              ErrorDetail(string='You do not have permission '
                                                 'to perform this action.',
                                          code='permission_denied')}, response.data)
        self.book_1.refresh_from_db()  # обновит из базы
